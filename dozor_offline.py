...
def parseArgs():
    ...
    parser.add_argument(
        "-m", "--master", help="EIGER master file", type=str, required=True
    )
    ...
    parser.add_argument(
        "-c", "--cut_off", help="cut off for hit rate calculation", type=int, default=5
    )
    parser.add_argument(
        "-o", "--output", help="output directory", type=str, default="dozor_res"
    )
...
def gen_dozor_dat(master_file, dozor_filename):
    ...
    config = {
        ...
        "pixel_max": 64000,
        "exposure": 0.200,
        "detector_distance": 420.028,
        "X-ray_wavelength": 0.9537,
        "orgx": 1056,
        "orgy": 1134,
        ...
        # "number_images": 10,
        # "name_template_image": "/opt/dozor/.test_data/testraster01_0008_??????.h5",
    }
...
if __name__ == "__main__":
    ...
    for i in range(nproc):
        ...
        p = multiprocessing.Process(
            ...
            args=(
                ...
                mask,
                start_tmp,
                min(end_img, start_tmp + img_per_proc - 1),
                output_file,
                cut_off,
                return_dict,
            ),
    ...
    perc = "%"
    print(
        "Found bragg spots in %d out of %d images and the hit rate is %.1f %s"
        % (hit_num, total_img, hit_num * 100.0 / total_img, perc)
    )
